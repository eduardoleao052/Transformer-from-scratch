...
class Model:
    def __init__(self, vocab_size: int) -> None:
        ...
        fcc1 = TemporalDense(vocab_size, 250)
        rnn1 = RNN(250, 250)
        rnn2 = RNN(250, 250)
        fcc2 = TemporalDense(250, vocab_size)  
        soft = TemporalSoftmax()
        self.layers = [fcc1,rnn1,rnn2,fcc2,soft]
    ...
    def load(self, path:str) -> None:
        ...
        for i, layer in enumerate(self.layers):
            layer.params = {key: np.array(value) for key, value in param_list[i].items()}
    ...
    def sample(self, seed:str, n_timesteps:int) -> list:
        ...
        inputs = np.zeros((1, len(seed), self.vocab_size))
        ...
        idxs = [np.random.choice(range(self.vocab_size), p = self.a.ravel())]
        ...
        for t in range(n_timesteps):   
            ...
            idx = np.random.choice(range(self.vocab_size), p = self.a.ravel())
    ...
    def test(self, n_timesteps:int, batch_size:int) -> int:
        ...
        @returns np.mean(test_losses) (int): mean of all test losses computed in this test
        """
        test_pointer = 0
        test_losses = []
        ...
        while test_pointer + 2 * (n_timesteps * batch_size ) + 1 < len(self.test_text):
            input_idxs = np.zeros([batch_size,n_timesteps])
            target_idxs = np.zeros([batch_size,n_timesteps])
            # store the index of every element in this batch:
            
            for b in range(batch_size):
                input_idxs[b,:] = [self.char_to_ix[ch] for ch in self.test_text[
                    test_pointer + ((b)*n_timesteps): test_pointer + ((b+1)*n_timesteps)
                    ]]
                target_idxs[b,:] = [int(self.char_to_ix[ch]) for ch in self.test_text[
                    1 + test_pointer + ((b)*n_timesteps): 1 + test_pointer + ((b+1)*n_timesteps)
                    ]]

            # build one-hot-encoded vectors for input:
            inputs = np.zeros((batch_size, n_timesteps, self.vocab_size))
    ...
    def _get_batch(self,pointer,n_timesteps,batch_size):
        input_idxs = np.zeros([batch_size,n_timesteps])
        target_idxs = np.zeros([batch_size,n_timesteps])
        for b in range(batch_size):
            input_idxs[b,:] = [self.char_to_ix[ch] for ch in self.train_text[
                pointer + ((b)*n_timesteps): pointer + ((b+1)*n_timesteps)
                ]]
            target_idxs[b,:] = [int(self.char_to_ix[ch]) for ch in self.train_text[
    ...
    def train(self, n_iter: int, n_timesteps: int, batch_size: int,learning_rate=1e-3,regularization=1e-3,patience = 7) -> None: 
        ...
        for t in range(n_iter):
            ...
            inputs, _, target_idxs = self._get_batch(pointer, n_timesteps, batch_size)
